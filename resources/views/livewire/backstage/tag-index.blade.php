    ...
    @if ($tags->isNotEmpty())
      <ul class="mt-3 grid grid-cols-1 gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <li class="col-span-1 flex shadow-sm rounded-md">
          <div class="flex-1 flex items-center justify-between border border-zinc-800 bg-zinc-600 rounded-md truncate">
            <div class="flex-1 px-4 py-2 text-sm leading-5 truncate">
              {{ $tag->name }}
              <p class="text-zinc-400 text-sm">/{{ $tag->slug }}</p>
            </div>
            <div class="shrink-0 pr-2">
              <a
                class="w-8 h-8 inline-flex items-center justify-center text-gray-400 rounded-full bg-transparent hover:text-zinc-500 focus:outline-none focus:text-gray-500 focus:bg-gray-100 transition ease-in-out duration-150"
