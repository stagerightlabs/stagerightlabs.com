    ...
    @foreach ($elements as $element)
      ...
      @if (is_array($element))
        @foreach ($element as $page => $url)
          ...
          @else
            <button wire:click="gotoPage({{ $page }})" class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-400 bg-cool-gray-700 border border-cool-gray-700 leading-5 hover:text-cool-gray-400 focus:z-10 focus:outline-none focus:border-blue-300 focus:ring-blue active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" aria-label="{{ __('pagination.goto_page', ['page' => $page]) }}">
              {{ $page }}
            </button>
          @endif
        @endforeach
