...
@section("content")
  ...
  @if ($document->series)
    <div
      class="mb-16 overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-600"
    >
      <div class="px-4 py-5 sm:p-6">
        ...
        <ul role="list" class="mt-4 list-inside list-disc text-lg/8">
          @foreach ($series[$document->series] as $post)
            ...
            @else
              <li class="">
                <a href="{{ $post["link"] }}" class="text-red-800 underline">
                  {{ $post["title"] }}
                </a>
              </li>
            @endif
...
@section("og")
  ...
  <meta
    ...
    content="{{ $document->date->format("c") }}"
  />
